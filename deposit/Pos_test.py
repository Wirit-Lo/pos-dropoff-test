...
def force_scroll_down(window, scroll_dist=-5):
    ...
    try:
        ...
        # จุดหมุน Mouse อยู่ค่อนไปทางขวา
    ...
    for i in range(max_scrolls + 1):
        ...
        if found_element:
            try:
                ...
                # Safe Zone: ถ้าปุ่มอยู่ต่ำกว่านี้ถือว่าเสี่ยง (เช่นโดน Taskbar บัง)
                safe_bottom_limit = win_rect.bottom - 80 # เผื่อระยะเพิ่มอีกนิด (จาก 70 เป็น 80)
                
                if elem_rect.bottom >= safe_bottom_limit:
                    log(f"   [!] เจอปุ่ม '{criteria}' แต่อยู่ต่ำมาก (ติดขอบล่าง) -> ดันขึ้นแรงๆ")
                    
                    # [FIX Logic Here] 
                    # เปลี่ยนจาก -4 เป็น -10 (หรือ -12) เพื่อให้มันดีดขึ้นมากลางจอเลย
                    # การขยับนิดเดียวเสียเวลาและอาจไม่พ้นขอบ
