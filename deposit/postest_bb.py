...
def run_smart_scenario(main_window, config):
    try:
        ...
        # [ส่วนที่เพิ่ม] อ่านค่า Config ประกัน
        add_insurance_flag = config['DEPOSIT_ENVELOPE'].get('AddInsurance', 'False')
        insurance_amt = config['DEPOSIT_ENVELOPE'].get('Insurance', '1000')
    ...
    # ใช้ตัวแปร is_registered เพื่อคุม Flow ต่อ (ถ้า True จะข้าม Popup จำนวน, ถ้า False จะเข้า Popup จำนวน)
    is_registered = str(add_insurance_flag).lower() in ['true', 'yes', 'on', '1']

    if is_registered:
        log(f"...Config สั่งให้ทำประกัน: วงเงิน {insurance_amt}...")
    ...
    # ถ้า is_registered (add_insurance_flag) เป็น False ระบบจะไหลไปเข้าเงื่อนไขถัดไป
    # ซึ่งจะจัดการ Popup จำนวน ตามที่คุณต้องการ
