...
def run_smart_scenario(main_window, config):
    ...
    time.sleep(step_delay)
    if not smart_click_with_scroll(main_window, "ซอง A3 เอกสาร", scroll_dist=scroll_dist):
        return
    time.sleep(step_delay)
    ...
    use_insurance_flow = str(add_insurance_flag).lower() in [
        'true', 'yes', 'on', '1']
    ...
    if use_insurance_flow:
        ...
        # ==========================================
        log(
            f"...Config เลือกทำประกัน -> กดปุ่ม (+) และกรอกวงเงิน {insurance_amt}...")
        ...
        if popup_window:
            try:
                ...
                if valid_edits:
                    ...
                    target_edit.type_keys("{DELETE}")  # เคลียร์ค่าเก่า
