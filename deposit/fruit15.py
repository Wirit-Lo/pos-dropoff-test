...
def process_receiver_address_selection(window, address_keyword, manual_data):
    ...
    if wait_for_text(window, "ข้อมูลผู้รับ", timeout=5):
        ...
        if found_popup:
            ...
            time.sleep(1.0)
            try:
                all_list_items = [i for i in window.descendants(
                    control_type="ListItem") if i.is_visible()]
                valid_items = [i for i in all_list_items if i.rectangle(
                ).top > 200 and i.rectangle().height() > 50]
                if valid_items:
...
def process_receiver_details_form(window, fname, lname, phone, is_manual_mode, manual_data):
    ...
    for i in range(3):
        ...
        if check_error_popup(window, delay=0.5):
            log(
                f"   [!] พบ Popup แจ้งเตือน (รอบที่ {i+1}) -> ปิดและดำเนินการต่อ")
...
def run_smart_scenario(main_window, config):
    try:
        category_name = "กล่องบรรจุผลไม้ขนาด M"
    ...
    for _ in range(6):
        try:
            ...
            if found_overlap:
                ...
                # วนกลับไปเช็คอีกรอบ (เผื่อมี Error เด้งตามมาหลังกดดำเนินการ)
                continue
    ...
    wait_until_id_appears(main_window, "ShippingService_361011", timeout=15)
    if find_and_click_with_rotate_logic(main_window, "ShippingService_361011"):
        main_window.type_keys("{ENTER}")
