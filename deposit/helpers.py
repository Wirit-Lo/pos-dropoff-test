...
def handle_car_tax_step(window, config_tax):
    """
    Step 5: จัดการหน้าภาษีรถยนต์ (ใช้ ID จริง: Element, Element1...7)
    """
    log("--- Step 5: คำนวณค่าภาษีรถยนต์ (Dynamic Form) ---")
    
    # 1. รอหน้าจอโหลด (รอคำว่า 'ประเภทรถ')
    wait_for_text(window, ["ประเภทรถ", "วันครบกำหนด"])

    # =======================================================
    # ส่วนที่ 1: เลือกประเภทรถหลัก (Main Type)
    # =======================================================
    main_type = config_tax.get('VehicleType', '')
    if main_type:
        log(f"   [Main] กำลังเลือกประเภทรถ: {main_type}")
        
        # 🔥 ใช้ ID='Element' (ตามที่คุณสแกนเจอ)
        # ฟังก์ชันนี้จะกดกล่อง 'Element' แล้ววนหาคำว่า main_type (Name) ข้างใน
        select_dropdown_using_pagedown(window, "Element", main_type)
        
        time.sleep(2.0) # รอให้กล่องลูกๆ (Element 1-7) โผล่มา
    else:
