...
################################# ธนาณัติ #################################
# --- แก้ไขใน helpers.py (handle_car_tax_step) ---

@strict_check
def handle_car_tax_step(window, config_tax):
    """
    Step 5: จัดการหน้าภาษีรถยนต์ (ใช้ Mapping แปลงชื่อไทยเป็น ID)
    """
    ...
    # =======================================================
    main_type_name = config_tax.get('VehicleType', '') # อ่านชื่อไทยจาก Config
    
    if main_type_name:
        log(f"   [Main] กำลังเลือกประเภทรถ: {main_type_name}")

        # 🔥 [ส่วนที่เพิ่ม] ตารางแปลงชื่อไทย -> ID (จากข้อมูลที่คุณดึงมา)
        # ใส่ให้ครบทุกคันตาม Log ที่คุณส่งมาครับ
        VEHICLE_ID_MAP = {
        ...
        # แปลงชื่อเป็น ID (ถ้าหาไม่เจอใน Map ให้ใช้ชื่อเดิมไปเสี่ยงดวงเอา)
        target_key = VEHICLE_ID_MAP.get(main_type_name, main_type_name)
        
        # ส่ง ID ไปให้ฟังก์ชันเลือก (มันจะแม่นกว่าหาด้วยชื่อไทยมาก)
        # Box ID: "Element" (ตามที่คุณแจ้ง)
    ...
    # ... (ส่วนที่ 2-4 เหมือนเดิม ไม่ต้องแก้) ...

    # =======================================================
    # ส่วนที่ 2: กรอกข้อมูล Dynamic (Element 1-5)
    # =======================================================
    
    # --- [Element 1] ขนาดซีซี (สำหรับ รย.1) ---
    cc_val = config_tax.get('EngineCC', '')
    if cc_val:
        # เช็คว่ามีกล่อง Element1 โผล่มาไหมก่อนกรอก
        if find_and_fill_smart(window, "ซีซี", "Element1", cc_val):
            log(f"   [Fill] กรอกซีซี (Element1): {cc_val}")

    # --- [Element 2] ปีที่จดทะเบียน (สำหรับ รย.1) ---
    year_val = config_tax.get('RegYear', '')
    if year_val:
        if find_and_fill_smart(window, "ปีที่จด", "Element2", year_val):
            log(f"   [Fill] กรอกปีจดทะเบียน (Element2): {year_val}")

    # --- [Element 3] น้ำหนัก (สำหรับ รย.2, รย.3) ---
    weight_val = config_tax.get('VehicleWeight', '')
    if weight_val:
        if find_and_fill_smart(window, "น้ำหนัก", "Element3", weight_val):
            log(f"   [Fill] กรอกน้ำหนัก (Element3): {weight_val}")

    # --- [Element 4] ประเภทเจ้าของ (Dropdown) ---
    owner_type = config_tax.get('OwnerType', '')
    if owner_type:
        # เช็คว่ามี Element4 ไหม (บางทีมันอาจจะไม่ขึ้นถ้าเลือกรถผิดประเภท)
        if len([c for c in window.descendants() if c.element_info.automation_id == "Element4"]) > 0:
            log(f"   [Select] เลือกประเภทเจ้าของ (Element4): {owner_type}")
            select_dropdown_using_pagedown(window, "Element4", owner_type)

    # --- [Element 5] ประเภทจักรยานยนต์ (Dropdown) ---
    moto_type = config_tax.get('MotorcycleType', '')
    if moto_type:
        # เช็คว่ามี Element5 ไหม
        if len([c for c in window.descendants() if c.element_info.automation_id == "Element5"]) > 0:
            log(f"   [Select] เลือกประเภท จยย. (Element5): {moto_type}")
            select_dropdown_using_pagedown(window, "Element5", moto_type)

    # =======================================================
    # ส่วนที่ 3: ค่าธรรมเนียม & วันครบกำหนด (Common)
    # =======================================================
    ...
    if need_fee:
        ...
        # รอแป๊บนึงเผื่อ Switch ยังไม่มา
        wait_until_id_appears(window, "SwitchThumb", timeout=3)
        click_element_by_id(window, "SwitchThumb")
    else:
        log("   [Switch] ไม่เปลี่ยนเล่ม (ใช้ค่าเดิม 'ไม่')")

    # --- [Element 7] วันครบกำหนดภาษี ---
