...
def process_special_services(window, services_str):
    ...
    if services_str and services_str.strip():
        ...
        for s in services_str.split(','):
            if s:
                smart_click(window, s.strip(), timeout=2)
    else:
...
def run_smart_scenario(main_window, config):
    try:
        ...
        register_flag = config['DEPOSIT_ENVELOPE'].get(
    ...
    time.sleep(step_delay)
    if not smart_click_with_scroll(main_window, "กล่องสำเร็จรูปแบบ กก.", scroll_dist=scroll_dist):
        return
    time.sleep(step_delay)
    ...
    target_service_id = "ShippingService_2582"
    ...
    if not is_registered:
        ...
        qty = config['PRODUCT_QUANTITY'].get(
            'Quantity', '1') if 'PRODUCT_QUANTITY' in config else '1'
        log(f"...เริ่มค้นหา Popup 'จำนวน' (จะใส่เลขจาก Config: {qty})...")

        # [แก้ไข] เปลี่ยนจาก time.sleep() ตายตัว เป็น Loop รอจนกว่าจะเจอ Popup จริงๆ
        popup_window = None
        ...
        for i in range(max_retries):
            ...
            if not temp_window:
                try:
                    app_top = Application(backend="uia").connect(
                        active_only=True).top_window()
            ...
            # 2. ถ้าเจอหน้าต่าง ต้องเช็คว่า "มีช่อง Edit หรือไม่" (สำคัญมาก)
            # เพื่อป้องกันเคสที่หน้าต่างมาแล้ว แต่ไส้ในยังโหลดไม่เสร็จ
            if temp_window:
                try:
                    ...
                    if edits and any(e.is_visible() for e in edits):
                        ...
                        log(
                            f"-> [Attempt {i+1}] เจอ Popup และช่อง Edit พร้อมใช้งาน: {popup_window.window_text()}")
        ...
        if popup_window:
            ...
            try:
                popup_window = Application(backend="uia").connect(
                    active_only=True).top_window()
            except:
                popup_window = main_window
        ...
        if popup_window:
            ...
            try:
                ...
                if visible_edits:
                    ...
                    valid_edits = [
                        e for e in visible_edits if e.rectangle().width() > 30]

                    if valid_edits:
                        target_edit = valid_edits[0]
                        log(
                            f"-> เป้าหมาย: {target_edit} (ID: {target_edit.element_info.automation_id})")
                    else:
